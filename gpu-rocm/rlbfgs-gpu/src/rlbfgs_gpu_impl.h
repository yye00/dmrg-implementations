#ifndef RLBFGS_GPU_IMPL_H
#define RLBFGS_GPU_IMPL_H
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::allocate_gpu_memory() {
    size_t mps_sz = max_site_size();

    d_mps_.assign(L_, nullptr);
    d_grad_.assign(L_, nullptr);
    d_g_prev_.assign(L_, nullptr);
    d_dir_.assign(L_, nullptr);
    d_trial_mps_.assign(L_, nullptr);
    d_trial_g_.assign(L_, nullptr);
    ...
    for (int i = 0; i < L_; i++) {
        ...
        HIP_CHECK(hipMalloc(&d_g_prev_[i], mps_sz * sizeof(Scalar)));
        HIP_CHECK(hipMalloc(&d_dir_[i], mps_sz * sizeof(Scalar)));
        HIP_CHECK(hipMalloc(&d_trial_mps_[i], mps_sz * sizeof(Scalar)));
        HIP_CHECK(hipMalloc(&d_trial_g_[i], mps_sz * sizeof(Scalar)));
    }
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::free_gpu_memory() {
    auto safe_free = [](void* p){ if (p) hipFree(p); };
    for (auto p : d_mps_)       safe_free(p);
    for (auto p : d_grad_)      safe_free(p);
    for (auto p : d_g_prev_)    safe_free(p);
    for (auto p : d_dir_)       safe_free(p);
    for (auto p : d_trial_mps_) safe_free(p);
    for (auto p : d_trial_g_)   safe_free(p);
    for (auto& hs : d_hist_s_) for (auto p : hs) safe_free(p);
    for (auto& hy : d_hist_y_) for (auto p : hy) safe_free(p);
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::set_mpo(const std::vector<Scalar*>& h_mpo_tensors) {
    upload_and_fuse_mpo(h_mpo_tensors);
}

template<typename Scalar>
void RLBFGSGPU<Scalar>::upload_and_fuse_mpo(const std::vector<Scalar*>& h_mpo_tensors) {
    int D = D_mpo_, d = d_;
    for (int i = 0; i < L_; i++) {
        int sz = D * d * d * D;
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::initialize_mps_random(double scale, int seed) {
    srand(seed);
    for (int i = 0; i < L_; i++) {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::right_canonicalize_mps() {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::update_left_H_env(int site) {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::update_right_H_env(int site) {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::build_initial_H_envs() {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::build_all_L_N_envs() {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::apply_heff(int site, const Scalar* d_in, Scalar* d_out) {
    int cL = chi_L(site);
    int cR = chi_R(site);
    int D = D_mpo_, d = d_;
    Scalar one = Traits::one(), zero_val = Traits::zero();
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::apply_norm(int site, const Scalar* d_in, Scalar* d_out) {
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::compute_energy_from_envs() {
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::compute_all_gradients() {
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::tangent_project_cores(std::vector<Scalar*>& cores) {
    Scalar one = Traits::one(), zero_v = Traits::zero(), neg_one = Traits::neg(one);
...
// Tangent-space real inner product: sum_i Re(<A_i, B_i>).
template<typename Scalar>
double RLBFGSGPU<Scalar>::tangent_inner_real(const std::vector<Scalar*>& A,
    ...
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        Scalar dot;
        // Traits::dot returns sum_j conj(A_j) * B_j (for complex); real part is the Riemannian inner.
        ROCBLAS_CHECK(Traits::dot((rocblas_handle)handle_, sz,
                                    A[i], 1, B[i], 1, &dot));
        if constexpr (Traits::is_complex) total += hipCreal(dot);
        else                               total += dot;
    }
    HIP_CHECK(hipStreamSynchronize(stream_));
    return total;
...
double RLBFGSGPU<Scalar>::tangent_norm_sq(const std::vector<Scalar*>& A) const {
    ...
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        RealType n;
        ROCBLAS_CHECK(Traits::nrm2((rocblas_handle)handle_, sz, A[i], 1, &n));
...
void RLBFGSGPU<Scalar>::tangent_scale(std::vector<Scalar*>& A, double alpha) {
    ...
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        ROCBLAS_CHECK(Traits::scal_real(handle_, sz, &a, A[i], 1));
    }
    HIP_CHECK(hipStreamSynchronize(stream_));
}
...
void RLBFGSGPU<Scalar>::tangent_axpy(double alpha,
    ...
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        ROCBLAS_CHECK(Traits::axpy(handle_, sz, &a, X[i], 1, Y[i], 1));
    }
    HIP_CHECK(hipStreamSynchronize(stream_));
}

template<typename Scalar>
void RLBFGSGPU<Scalar>::tangent_copy(const std::vector<Scalar*>& Src,
                                     std::vector<Scalar*>& Dst) {
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        HIP_CHECK(hipMemcpyAsync(Dst[i], Src[i], sz * sizeof(Scalar),
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::tangent_zero(std::vector<Scalar*>& A) {
    for (int i = 0; i < L_; i++) {
        int sz = chi_L(i) * d_ * chi_R(i);
        HIP_CHECK(hipMemsetAsync(A[i], 0, sz * sizeof(Scalar), stream_));
    }
    HIP_CHECK(hipStreamSynchronize(stream_));
}
...
//
// INPUT:  d_grad_[i] holds the current projected Riemannian gradient g_k
//         in the tangent space at d_mps_ (current iterate).
// OUTPUT: d_dir_[i] holds the search direction d_k = -H_k * g_k.
//
// For each stored pair (s_j, y_j, rho_j), transport s_j and y_j to the
// current tangent space by re-projection (via tangent_project_cores),
// then apply the standard two-loop (Nocedal & Wright, Alg. 7.4).
//
// We use the scratch arrays d_g_prev_ and d_trial_g_ as transported-vector
// workspace (two slots, since after the first loop we only need the
// already-transported vectors).  Simpler approach: allocate a single
// extra workspace for the transported copy on the fly.
// ============================================================================

template<typename Scalar>
void RLBFGSGPU<Scalar>::lbfgs_two_loop() {
    // d_dir_ = g_k as starting point for q.
    tangent_copy(d_grad_, d_dir_);

    if (hist_count_ == 0) {
        // No history: d_dir_ = -g_k (steepest descent).
        tangent_scale(d_dir_, -1.0);
        return;
    }

    // Pre-allocate per-pair transported-(s,y) workspace.  To avoid
    // ballooning memory, we keep only ONE transported slot and transport
    // just-in-time in each loop iteration.  That costs an extra projection
    // but is cheap relative to the gradient step itself.
    //
    // Order: pairs in the ring buffer are stored at indices
    //   (hist_head_ - hist_count_ + k) mod history_size   for k = 0..hist_count_-1
    // With history_size == hist_count_ == pairs_max, "oldest first" is
    //   idx(k) = (hist_head_ + k) mod hist_count_
    // after the ring has wrapped.  Before wrapping (hist_count_ < history_size),
    // the oldest is at index 0 and the newest at hist_count_-1.
    //
    // To sidestep both cases cleanly, we keep a logical linear ordering:
    //   oldest ... newest == ring_index(0) ... ring_index(hist_count_-1)
    // where:
    auto ring_index = [&](int k) -> int {
        // k in [0, hist_count_); 0 is oldest, hist_count_-1 is newest.
        int sz = (int)h_hist_rho_.size();
        if (hist_count_ < sz) return k;
        // ring wrapped: oldest lives at hist_head_
        return (hist_head_ + k) % sz;
    };

    std::vector<double> alphas(hist_count_, 0.0);
    // --- first loop: newest → oldest ---
    for (int k = hist_count_ - 1; k >= 0; k--) {
        int j = ring_index(k);
        // Transport s_j, y_j onto current tangent space (reuses d_g_prev_/d_trial_g_ as scratch).
        tangent_copy(d_hist_s_[j], d_g_prev_);
        tangent_copy(d_hist_y_[j], d_trial_g_);
        tangent_project_cores(d_g_prev_);
        tangent_project_cores(d_trial_g_);

        double sy = tangent_inner_real(d_g_prev_, d_trial_g_);
        double rho = (sy > 0.0) ? (1.0 / sy) : 0.0;

        double sq = tangent_inner_real(d_g_prev_, d_dir_);
        double a  = rho * sq;
        alphas[k] = a;

        // q -= a * y_j   (here q lives in d_dir_)
        tangent_axpy(-a, d_trial_g_, d_dir_);
    }

    // Initial Hessian scaling gamma = <s_last, y_last> / <y_last, y_last>
    // Compute using the newest pair transported.
    double gamma = 1.0;
    {
        int j = ring_index(hist_count_ - 1);
        tangent_copy(d_hist_s_[j], d_g_prev_);
        tangent_copy(d_hist_y_[j], d_trial_g_);
        tangent_project_cores(d_g_prev_);
        tangent_project_cores(d_trial_g_);
        double sy = tangent_inner_real(d_g_prev_, d_trial_g_);
        double yy = tangent_inner_real(d_trial_g_, d_trial_g_);
        if (yy > 0.0) gamma = sy / yy;
    }
    tangent_scale(d_dir_, gamma);

    // --- second loop: oldest → newest ---
    for (int k = 0; k < hist_count_; k++) {
        int j = ring_index(k);
        tangent_copy(d_hist_s_[j], d_g_prev_);
        tangent_copy(d_hist_y_[j], d_trial_g_);
        tangent_project_cores(d_g_prev_);
        ...
        double yr = tangent_inner_real(d_trial_g_, d_dir_);
        double b  = rho * yr;
        // r += (alpha_j - b) * s_j
        tangent_axpy(alphas[k] - b, d_g_prev_, d_dir_);
    }

    // d_dir_ currently holds H_k g_k; flip sign for the descent direction.
    tangent_scale(d_dir_, -1.0);
}

// ============================================================================
// Retraction helpers for line search.
//   trial_energy_at(alpha): write d_trial_mps_[i] = R(d_mps_[i] + alpha*d_dir_[i])
//                            via right-canonicalization on the trial copy.
//   commit_trial(): swap d_mps_ ↔ d_trial_mps_, rebuild envs.
// ============================================================================
...
double RLBFGSGPU<Scalar>::trial_energy_at(double alpha) {
    ...
    HIP_CHECK(hipStreamSynchronize(stream_));
    // Temporarily install trial as current, canonicalize, compute energy, restore.
    std::swap(d_mps_, d_trial_mps_);
    right_canonicalize_mps();
    normalize_site0();
    build_initial_H_envs();
    double E = compute_energy_from_envs();
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::commit_trial() {
    std::swap(d_mps_, d_trial_mps_);
    build_initial_H_envs();
    build_all_L_N_envs();
}
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::lanczos_eigensolver(int site, Scalar* d_theta, int theta_size) {
    int n = theta_size;
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::svd_split_single_site(int site, Scalar* d_theta, char direction) {
    int cL = chi_L(site);
    int cR = chi_R(site);
    int m, n_svd;
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::optimize_site_single(int site, char direction) {
    int cL = chi_L(site), cR = chi_R(site);
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::dmrg1_sweep_LR() {
    double energy = 0.0;
...
template<typename Scalar>
double RLBFGSGPU<Scalar>::dmrg1_sweep_RL() {
    double energy = 0.0;
...
// ============================================================================
// Driver: optional DMRG1 warmstart, then R-LBFGS loop with Armijo line search
// ============================================================================

template<typename Scalar>
double RLBFGSGPU<Scalar>::run(const Config& cfg) {
    ...
    // 2) R-LBFGS setup: allocate history buffers on first call.
    if ((int)d_hist_s_.size() != cfg.history_size) {
        // Free any prior buffers.
        for (auto& hs : d_hist_s_) for (auto p : hs) if (p) hipFree(p);
        for (auto& hy : d_hist_y_) for (auto p : hy) if (p) hipFree(p);
        d_hist_s_.assign(cfg.history_size, std::vector<Scalar*>(L_, nullptr));
        d_hist_y_.assign(cfg.history_size, std::vector<Scalar*>(L_, nullptr));
        h_hist_rho_.assign(cfg.history_size, 0.0);
        for (int j = 0; j < cfg.history_size; j++) {
            for (int i = 0; i < L_; i++) {
                size_t sz = max_site_size();
                HIP_CHECK(hipMalloc(&d_hist_s_[j][i], sz * sizeof(Scalar)));
                HIP_CHECK(hipMalloc(&d_hist_y_[j][i], sz * sizeof(Scalar)));
            }
        }
        hist_count_ = 0;
        hist_head_ = 0;
    ...
    // Initial gradient + energy.
    build_initial_H_envs();
    build_all_L_N_envs();
    double E = compute_all_gradients();
    tangent_project_cores(d_grad_);
    double gnorm = std::sqrt(tangent_norm_sq(d_grad_));

    if (cfg.verbose) {
        std::cout << "[pre-LBFGS] E=" << std::fixed << std::setprecision(12) << E
                  << "  |g|=" << std::scientific << std::setprecision(3) << gnorm
                  << std::endl;
    }
    if (gnorm < cfg.grad_tol) {
        if (cfg.verbose) std::cout << "[skip LBFGS] |g| < " << cfg.grad_tol
    ...
    for (int step = 1; step <= cfg.max_epochs; step++) {
        // 2a) Search direction d_k = -H_k g_k.
        lbfgs_two_loop();

        // 2b) Descent guard: <g, d> must be negative.
        double slope = tangent_inner_real(d_grad_, d_dir_);
        bool fallback = false;
        if (slope >= 0.0) {
            // L-BFGS direction not descent; reset and use steepest descent.
            hist_count_ = 0;
            hist_head_ = 0;
            tangent_copy(d_grad_, d_dir_);
            tangent_scale(d_dir_, -1.0);
            slope = tangent_inner_real(d_grad_, d_dir_);
            fallback = true;
        }

        // 2c) Armijo backtracking line search.
        double alpha = cfg.alpha_init;
        double E_trial = E;
        bool ls_ok = false;
        for (int ls = 0; ls < cfg.ls_max_iter; ls++) {
            E_trial = trial_energy_at(alpha);
            if (std::isfinite(E_trial) &&
                E_trial <= E + cfg.c1 * alpha * slope) {
                ls_ok = true;
                break;
            }
            alpha *= cfg.beta;
            if (alpha < cfg.min_alpha) break;
        }

        if (!ls_ok) {
            if (cfg.verbose) {
                std::cout << "[step " << step << "] line search failed, alpha="
                          << std::scientific << alpha << " — stopping" << std::endl;
            }
            n_epochs_done_ = step - 1;
            break;
        }

        // 2d) Save old gradient (still on tangent space at d_mps_) before committing.
        tangent_copy(d_grad_, d_g_prev_);
        // Save step s_k = alpha * d_dir_ (computed from the direction at d_mps_).
        // We store it in d_trial_g_ temporarily as workspace, then push to history
        // after we have the new gradient.
        // But first, we need to know the transport of s_k to the new tangent space
        // (the CPU reference stores s transported to X_{k+1}).
        //
        // Plan:
        //   1) Compute s_k = alpha * d_dir_ (still at old tangent).
        //   2) Commit trial → new d_mps_.
        //   3) Rebuild envs, compute new gradient g_{k+1}.
        //   4) Project s_k onto new tangent space.
        //   5) y_k = g_{k+1} - project(g_k, new_tangent).
        //   6) Push (s_k, y_k, 1/<s,y>) with cautious update.

        // Step 1: s_k = alpha * d_dir_   (store in d_trial_g_ as workspace)
        tangent_copy(d_dir_, d_trial_g_);
        tangent_scale(d_trial_g_, alpha);

        // Step 2: commit trial.
        commit_trial();

        // Step 3: new gradient.
        double E_new = compute_all_gradients();
        tangent_project_cores(d_grad_);
        double gnorm_new = std::sqrt(tangent_norm_sq(d_grad_));

        // Step 4: project s_k onto new tangent space (s_k is in d_trial_g_).
        tangent_project_cores(d_trial_g_);
        // Step 5: project old gradient onto new tangent space (g_prev).
        tangent_project_cores(d_g_prev_);
        // y_k = g_new - g_prev_projected. Use d_dir_ as workspace for y_k.
        tangent_copy(d_grad_, d_dir_);          // d_dir_ = g_new
        tangent_axpy(-1.0, d_g_prev_, d_dir_);  // d_dir_ = g_new - g_prev_proj

        // Step 6: cautious update.
        double sy = tangent_inner_real(d_trial_g_, d_dir_);
        double s_norm = std::sqrt(tangent_norm_sq(d_trial_g_));
        double y_norm = std::sqrt(tangent_norm_sq(d_dir_));
        bool push_pair = (sy > cfg.cautious_eps * std::max(s_norm * y_norm, 1e-30));
        if (push_pair) {
            // Ring buffer: overwrite oldest (at hist_head_) if full, else append.
            int sz = cfg.history_size;
            int slot = hist_head_;
            tangent_copy(d_trial_g_, d_hist_s_[slot]);
            tangent_copy(d_dir_,     d_hist_y_[slot]);
            h_hist_rho_[slot] = 1.0 / sy;
            if (hist_count_ < sz) {
                // Not full: append; oldest stays at 0; head advances.
                hist_count_++;
                hist_head_ = hist_count_ % sz;
            } else {
                hist_head_ = (hist_head_ + 1) % sz;
            }
        }
...
template<typename Scalar>
void RLBFGSGPU<Scalar>::get_mps(std::vector<std::vector<Scalar>>& h_mps) const {
    h_mps.resize(L_);
    for (int i = 0; i < L_; i++) {
