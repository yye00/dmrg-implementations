...
DMRG2GPUOpt<Scalar>::DMRG2GPUOpt(int L, int d, int chi_max, int D_mpo, double tol)
    ...
    // Contraction intermediates: D*d^2*chi_max^2 — disjoint buffers per stream
    ...
    // Round-16 D12: device-pointer-mode Lanczos scalars + α/β arrays.
    ...
    HIP_CHECK(hipMalloc(&d_alpha_dev_,   max_lanczos_iter_ * sizeof(double)));
    HIP_CHECK(hipMalloc(&d_beta_dev_,    max_lanczos_iter_ * sizeof(double)));
    ...
    {
        Scalar one_h = Traits::one(), zero_h = Traits::zero(), neg_h = Traits::neg(Traits::one());
        HIP_CHECK(hipMemcpy(d_const_one_,    &one_h,  sizeof(Scalar), hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_const_zero_,   &zero_h, sizeof(Scalar), hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_const_neg_one_,&neg_h,  sizeof(Scalar), hipMemcpyHostToDevice));
    }

    // Batched GEMM pointer arrays: D*d^2 batches for two-site — main + env
    // stream variants so env_update and apply_heff/absorb don't clobber each
    // other's pointer tables.
    ...
    // rocsolver_dsteqr scratch (replaces 2× host LAPACK dstev_ in Lanczos).
    ...
    // On-device Rayleigh-Ritz scratch (round-7 H6 fix).
...
void DMRG2GPUOpt<Scalar>::apply_heff_two_site(int site, const Scalar* d_theta_in, Scalar* d_result) {
    ...
    // Step 1: Batched GEMM — contract L_env with theta. Round-15 follow-up
    // H2-opt-host-batch-pointer fix: GPU kernels replace 3× hipMemcpyAsync
    // H2D pattern. Mirrors dmrg2-gpu impl :789-820 sibling pattern.
    {
        if (sparse_s1) {
            ...
            hipLaunchKernelGGL(setup_batch_ptrs_wd_twosite<Scalar>,
...
void DMRG2GPUOpt<Scalar>::update_left_env(int site) {
    ...
    // it can overlap with the trailing scale on stream_. Caller must make
    // stream_env_ wait on event_canon_ready_ first.
    ...
    {
        // Round-15 follow-up H2-opt-host-batch-pointer fix.
...
void DMRG2GPUOpt<Scalar>::update_right_env(int site) {
    ...
    // Round-15 follow-up H2-opt-host-batch-pointer fix: setup_batch_ptrs_sw
    // (s-major A, w-major B) replaces 3× hipMemcpyAsync H2D pattern.
    {
...
double DMRG2GPUOpt<Scalar>::lanczos_eigensolver(int site, Scalar* d_theta, int theta_size) {
    ...
    // Round-16 D12 port: device-pointer-mode Lanczos (mirrors dmrg-gpu's
    // round-7 H10 + dmrg-gpu-opt's round-16 D12 port). α/β stored in
    // d_alpha_dev_/d_beta_dev_; rocsolver_dsteqr reads them directly.
    ...
    double norm;
    HIP_CHECK(hipMemcpyAsync(&norm, d_nrm2_result_, sizeof(double),
    ...
    for (iter = 0; iter < max_iter; iter++) {
        ...
        // w = H|v_i> (apply_heff_two_site in HOST pointer mode)
        ...
        if (iter >= 4 && iter % 3 == 0) {
            ...
            HIP_CHECK(hipMemcpyAsync(d_steqr_D_, d_alpha_dev_, ncheck * sizeof(double),
                                      hipMemcpyDeviceToDevice, stream_));
            HIP_CHECK(hipMemcpyAsync(d_steqr_E_, d_beta_dev_, ncheck * sizeof(double),
                                      hipMemcpyDeviceToDevice, stream_));
    ...
    HIP_CHECK(hipMemcpyAsync(d_steqr_D_, d_alpha_dev_, niter * sizeof(double),
                              hipMemcpyDeviceToDevice, stream_));
    HIP_CHECK(hipMemcpyAsync(d_steqr_E_, d_beta_dev_, niter * sizeof(double),
                              hipMemcpyDeviceToDevice, stream_));
    ...
    double energy;
    HIP_CHECK(hipMemcpy(&h_lapack_info, d_steqr_info_, sizeof(rocblas_int), hipMemcpyDeviceToHost));
...
// ============================================================================
// SVD bond splitting — on-device default path (rocsolver_gesvd_auto +
// on-device truncation/scale). Replaces the prior host-LAPACK
// svd_split_fallback that did D2H of theta + host gesvd + scale on CPU +
// H2D of MPS — exactly the per-sweep PCIe traffic we eliminated for the
// other -gpu-opt variants in 8dbd1b8.
// ============================================================================
...
void DMRG2GPUOpt<Scalar>::svd_split_fallback(int site, Scalar* d_theta, char direction) {
    ...
    if (direction == 'R') {
        ...
        // MPS[site] = U is queued. Signal env stream so update_left_env(site)
        // can run concurrently with the trailing S*Vh -> MPS[site+1] scale.
        ...
        // do the U*S -> MPS[site] absorb on stream_ concurrently with
        ...
        // U*S → MPS[site] (scale columns of U by S on device).
        allocate_mps_tensor(site, cL, new_k);
        {
            int total = m * new_k;
...
double DMRG2GPUOpt<Scalar>::block_davidson_eigensolver(int site, Scalar* d_theta, int theta_size) {
    ...
    // For tiny systems, use Lanczos fallback (its own t_lanczos_ pair).
    if (dim <= 2 * b) {
        return lanczos_eigensolver(site, d_theta, theta_size);
    }

    // Round-17 H1-opt-PhaseTimer-prop: dedicated t_davidson_ panel (R15
    // shared t_lanczos_, contradicting the header comment). Begin AFTER
    // the Lanczos fallback short-circuit; Lanczos has its own pair.
    ...
    for (int iteration = 0; iteration < max_iter; iteration++) {
        // Rayleigh-Ritz: H_proj = V^H @ AV  -> (k, k), into d_dav_work2_
        // (becomes the eigenvector buffer in-place after rocsolver_syevd).
        // Round-7 H6 — replaces the host-LAPACK syev roundtrip.
        ROCBLAS_CHECK(Traits::gemm(rocblas_h_,
            Traits::op_h, rocblas_operation_none,
            k, k, dim, &one, V, dim, AV, dim, &zero_val, d_dav_work2_, k));

        // On-device Hermitian eigendecomposition (uplo=upper; the projected
        // matrix is exactly Hermitian up to roundoff, no explicit symmetrize
        // needed).
        ...
        // Single-double D2H of lowest eigenvalue + info for control flow.
        double energy;
        ...
        // D2H lowest b eigenvalues for residual scaling below.
        ...
        // x0 = V @ eigvecs[:, 0] -> (dim, 1) (eigvecs in d_dav_work2_)
        ROCBLAS_CHECK(Traits::gemv(rocblas_h_, rocblas_operation_none,
        ...
        // Orthogonalize new vectors against V. d_dav_work2_ still holds
        // the eigvecs from syevd; route the overlap matrix through an
        // offset of d_dav_work_ so we don't trash them.
        ...
        if (k + n_good > max_sub) {
            ...
            // already in d_dav_work2_ from rocsolver_syevd; we preserved
            // them by offsetting the overlap matrix in d_dav_work_.
            ROCBLAS_CHECK(Traits::gemm(rocblas_h_,
                rocblas_operation_none, rocblas_operation_none,
...
double DMRG2GPUOpt<Scalar>::optimize_bond(int site, char direction) {
    ...
    // (profile=false) skips these syncs and lets svd_split_fallback queue
    // behind the eigensolver via stream ordering — significant speedup.
...
double DMRG2GPUOpt<Scalar>::sweep_left_to_right() {
    // Dual-stream pipeline (round-6 J2 port from dmrg-gpu pattern):
    //   stream_     : Lanczos/Davidson + SVD + trailing scale (S*Vh into MPS[site+1])
    //   stream_env_ : update_left_env(site) -> L[site+1]
    // svd_split_fallback records event_canon_ready_ as soon as MPS[site] = U
    // is queued; update_left_env runs concurrently with the trailing scale.
    double energy = 0.0;
    ...
    for (int site = 0; site < L_ - 1; site++) {
        ...
        // Env timing (gated by opts_.profile). When off, full dual-stream
        // overlap; when on, briefly serialize on stream_env_ for accurate
        // host-side timing.
