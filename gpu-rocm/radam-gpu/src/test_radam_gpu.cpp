...
// ============================================================================
int test_heisenberg(int L, int chi_max, const RAdamGPU<double>::Config& cfg,
                    bool cpu_svd) {
    int d = 2;
    int D_mpo = 5;

    printf("======================================\n");
    printf("RADAM-GPU Heisenberg Test (float64)\n");
    printf("======================================\n");
    printf("  L=%d, d=%d, chi_max=%d, D_mpo=%d\n", L, d, chi_max, D_mpo);
    printf("  max_epochs=%d, warmup=%d, lr=%.2e\n",
           cfg.max_epochs, cfg.n_warmup, cfg.lr);
    printf("  b1=%.3f, b2=%.4f, eps=%.1e, cosine=%d\n",
           cfg.beta1, cfg.beta2, cfg.eps, (int)cfg.cosine_lr);
    ...
    RAdamGPU<double> opt(L, d, chi_max, D_mpo, 1e-10);
...
// TFIM test (real)
// ============================================================================
int test_tfim(int L, int chi_max, const RAdamGPU<double>::Config& cfg,
              bool cpu_svd, double J, double h_field) {
    int d = 2;
    int D_mpo = 3;

    printf("======================================\n");
    printf("RADAM-GPU TFIM Test (float64)\n");
    printf("======================================\n");
    printf("  L=%d, d=%d, chi_max=%d, D_mpo=%d\n", L, d, chi_max, D_mpo);
    printf("  J=%.4f, h=%.4f\n", J, h_field);
    printf("  max_epochs=%d, warmup=%d, lr=%.2e\n",
...
// ============================================================================
int test_josephson(int L, int chi_max, const RAdamGPU<Complex>::Config& cfg,
                   bool cpu_svd, int n_max, double E_J, double E_C, double phi_ext) {
    int d = 2 * n_max + 1;
    int D_mpo = 4;

    printf("======================================\n");
    printf("RADAM-GPU Josephson Test (complex128)\n");
    printf("======================================\n");
    ...
    RAdamGPU<Complex> opt(L, d, chi_max, D_mpo, 1e-10);
    opt.set_cpu_svd(cpu_svd);
    opt.initialize_mps_random();
...
int main(int argc, char** argv) {
    ...
    bool cpu_svd = true;   // CPU SVD default (faster at small chi)
    bool quiet = false;
    int log_every = 10;
...

    {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (a == "--cosine") { cosine_lr = true; continue; }
            ...
            if (a == "--lr" && i+1 < argc) { lr = std::atof(argv[++i]); continue; }
            if (a == "--b1" && i+1 < argc) { beta1 = std::atof(argv[++i]); continue; }
            if (a == "--b2" && i+1 < argc) { beta2 = std::atof(argv[++i]); continue; }
            if (a == "--eps" && i+1 < argc) { eps = std::atof(argv[++i]); continue; }
