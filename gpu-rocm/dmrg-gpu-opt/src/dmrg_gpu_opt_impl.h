...
void DMRGGPUOpt<Scalar>::apply_heff(int site, const Scalar* d_theta_in, Scalar* d_result) {
    ...
    // Round-15 H1-opt-PhaseTimer-prop: panel was inert until this round
    // (round-7 wired init/report; no .begin/.end sites). Mirrors round-14
    // pdmrg-gpu-opt fix.
...
double DMRGGPUOpt<Scalar>::lanczos_eigensolver(int site, Scalar* d_theta) {
    ...
    // Round-16 D12 port: Lanczos main loop runs in device-pointer mode with
    // α/β stored on-device in d_alpha_dev_/d_beta_dev_. Mirrors dmrg-gpu's
    // round-7 H10 pattern. Tridiagonal solve (rocsolver_dsteqr) reads α/β
    // directly from device — no per-iter PCIe roundtrip.

    // v[0] = theta / ||theta||  (device-pointer mode)
    {
    ...
    for (iter = 0; iter < max_iter; iter++) {
        ...
        // w = H|v_i> (apply_heff in HOST pointer mode — uses host-stack &one/&zero gemms)
        apply_heff(site, d_vi, d_heff_result_);

        // BLAS-1 ops in DEVICE pointer mode. PointerModeGuard restores host
        // mode on scope exit (incl. throw), so apply_heff next iter sees host.
        {
            PointerModeGuard pm_guard(rocblas_h_, rocblas_pointer_mode_device);

            // alpha_i = <v_i|w>  → d_dot_result_
            ROCBLAS_CHECK(Traits::dot(rocblas_h_, n, d_vi, 1, d_heff_result_, 1, d_dot_result_));
            // Process: store Re(dot_result) to d_alpha_dev_[iter], emit -alpha into d_neg_alpha_
            ...
            if (opts_.fuse_lanczos) {
                ...
                // d_neg_alpha_ and d_neg_beta_scalars_+(iter-1) are device pointers.
                ...
                // w -= beta_{i-1} * v_{i-1} (device pointer from d_neg_beta_scalars_)
                if (iter > 0) {
            ...
            // Full reorthogonalization via gemv (device pointer mode for constants)
            if (iter > 0) {
            ...
            // beta_i = ||w||  → d_nrm2_result_
            ROCBLAS_CHECK(Traits::nrm2(rocblas_h_, n, d_heff_result_, 1, d_nrm2_result_));
            // Process: store β to d_beta_dev_[iter], compute 1/β to d_inv_nrm_, emit -β to d_neg_beta_scalars_[iter]
        ...
        }  // PointerModeGuard restores host mode

        // Convergence check every 3 iterations after iter >= 4. Both the
        // beta-tol check and the dsteqr eigenvalue check now run on device-
        // resident α/β (no per-iter H2D from a host buffer).
        if (iter >= 4 && iter % 3 == 0) {
            HIP_CHECK(hipStreamSynchronize(stream_));
            int ncheck = iter + 1;
            // Check for invariant subspace (beta < tol) on device
    ...
    // Final tridiagonal solve on device-resident α/β.
    ...
    // Ritz coefficients = first column of d_steqr_C_; promote to Scalar.
    if constexpr (std::is_same_v<Scalar, double>) {
        HIP_CHECK(hipMemcpyAsync(d_ritz_coeffs_, d_steqr_C_, niter * sizeof(double),
                                  hipMemcpyDeviceToDevice, stream_));
    } else {
        int blk = (niter + 63) / 64;
    ...
    // Final theta = V * ritz_coeffs + normalize (device pointer mode)
    {
        PointerModeGuard pm_guard(rocblas_h_, rocblas_pointer_mode_device);
        ROCBLAS_CHECK(Traits::gemv(rocblas_h_, rocblas_operation_none,
