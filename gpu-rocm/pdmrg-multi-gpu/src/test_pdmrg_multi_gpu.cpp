...
// ============================================================================
// Transverse-Field Ising Model MPO (real, D=3)
// ============================================================================
...
int test_heisenberg(int L, int chi_max, int n_outer, int n_devices,
    ...
    int avail;
    hipGetDeviceCount(&avail);

    if (!quiet) {
        printf("======================================\n");
        printf("PDMRG Multi-GPU Heisenberg Test (float64)\n");
        printf("======================================\n");
        printf("  L=%d, d=%d, chi_max=%d, D_mpo=%d\n", L, d, chi_max, D_mpo);
        printf("  devices=%d (available=%d), outer=%d, local=%d, warmup=%d\n",
               n_devices, avail, n_outer, n_local, n_warmup);
    ...
    PDMRGMultiGPU<double> pdmrg(L, d, chi_max, D_mpo, n_devices, 1e-10);
...
int test_tfim(int L, int chi_max, int n_outer, int n_devices,
    ...
    if (!quiet) {
        printf("======================================\n");
        printf("PDMRG Multi-GPU TFIM Test (float64)\n");
        printf("======================================\n");
        printf("  L=%d, d=%d, chi_max=%d, D_mpo=%d\n", L, d, chi_max, D_mpo);
        printf("  devices=%d (available=%d), outer=%d, local=%d, warmup=%d\n",
               n_devices, avail, n_outer, n_local, n_warmup);
...
int test_josephson(int L, int chi_max, int n_outer, int n_devices,
    ...
    if (!quiet) {
        printf("======================================\n");
        printf("PDMRG Multi-GPU Josephson Test (complex128)\n");
        printf("======================================\n");
    ...
    PDMRGMultiGPU<Complex> pdmrg(L, d, chi_max, D_mpo, n_devices, 1e-10);
    pdmrg.set_cpu_svd(!gpu_svd);
    pdmrg.set_rsvd(use_rsvd);
    pdmrg.set_quiet(quiet);
    pdmrg.initialize_mps_random();
