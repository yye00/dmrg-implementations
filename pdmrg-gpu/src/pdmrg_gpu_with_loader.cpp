...
// make_complex and MPOBase are defined in loaded_mpo.hpp
...
// MPOBase is defined in loaded_mpo.hpp
...
// ============================================================================
// Environment Tensors on GPU (hipTensor-based, from working dmrg_with_environments.cpp)
// ============================================================================
// Uses hipTensor for all contractions - proven correct for both Heisenberg and
// complex models. Each env update decomposes into 3 sequential hipTensor contractions.
...
public:
    ...
    //
    // Full contraction via 3 sequential hipTensor contractions:
    ...
    //
    // Full contraction via 3 sequential hipTensor contractions:
    // 1. temp1[a,s,w',b*] = A[a,s,b] * R[b,w',b*]  (contract over b)
    // 2. temp2[a,w,s',b*] = temp1[a,s,w',b*] * W[w,s,s',w']  (contract over s,w')
    // 3. R_new[a,w,a*] = temp2[a,w,s',b*] * conj(A[a*,s',b*])  (contract over s',b*)
    // -----------------------------------------------------------------------
...
public:
          ...
          verbose_debug(debug)
    {
        ...
        bond_dims.resize(L + 1);
        bond_dims[0] = mps_loaded[0].D_left;  // Should be 1
        for (int i = 0; i < L; i++) {
        ...
        for (int i = 0; i < L; i++) {
            ...
            // Convert HostComplex (std::complex<double>) to Complex (hipDoubleComplex)
            for (int j = 0; j < size; j++) {
        ...
        // Skip canonicalization for loaded MPS (already in proper format)
        std::cout << "✓ Skipping canonicalization (MPS loaded from file)\n";
    ...
    double run() {
        ...
        for (int sweep = 0; sweep < n_sweeps; sweep++) {
            ...
            // Use Lanczos energy from last site optimization instead of
            // expensive O(D^5) CPU transfer-matrix contraction
...
private:
    ...
    double optimize_site(int site, bool move_right) {
        ...
        // Fixed Lanczos iterations (matching proven working dmrg_with_environments.cpp)
...
int main(int argc, char** argv) {
    ...
    int L = 12;
    int d = 2;  // Physical dimension (will be updated if loading from file)
    int max_D = 100;
    int n_sweeps = 10;
    ...
    if (use_loaded_mps) {
        ...
        if (mps_file.find("josephson") != std::string::npos) {
            model = "josephson";
        } else if (mps_file.find("heisenberg") != std::string::npos) {
            model = "heisenberg";
        }
    }
