...
void StreamSegment::build_environments_from_mps() {
    // Build all environments from loaded MPS tensors
    // This is critical for computing correct energy after MPS loading

    printf("[Stream %d] Building environments from MPS\\n", id_);
    ...
    // L_envs[0]: left boundary (no sites to left)
    {
        ...
        std::vector<double> h_L_env(size_L, 0.0);
        h_L_env[0] = 1.0;  // Identity at (w=0, a=0, a=0)
        HIP_CHECK(hipMemcpy(d_L_envs_[0], h_L_env.data(),
                           size_L * sizeof(double), hipMemcpyHostToDevice));
    }
    ...
    for (int i = 0; i < num_sites_ - 1; i++) {
        ...
        // 3-step contraction (same as rebuild_right_boundary_env)
        // Step 1: temp1[w, a', s, b] = sum_a L[a, w, a'] * A[a, s, b]
        {
            int64_t extentL[] = {(int64_t)chi_L, (int64_t)D_mpo_, (int64_t)chi_L};
            int64_t extentA[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)chi_R};
        ...
        // Step 3: L_new[b, wp, b'] = sum_{a',s'} temp2[a', s', b, wp] * A*[a', s', b']
        {
            int64_t extent_temp2[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)chi_R, (int64_t)D_mpo_};
            int64_t extentA[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)chi_R};
            int64_t extent_L_new[] = {(int64_t)chi_R, (int64_t)D_mpo_, (int64_t)chi_R};

            int32_t modes_temp2[] = {0, 1, 2, 3};
            int32_t modesA[] = {0, 1, 4};
            int32_t modes_L_new[] = {2, 3, 4};

            hiptensor_contract(d_temp2, 4, extent_temp2, modes_temp2,
                             d_A, 3, extentA, modesA,
                             d_L_out, 3, extent_L_new, modes_L_new,
                             1.0, 0.0);
        }
    ...
    for (int i = num_sites_ - 2; i >= 0; i--) {
        int chi_L = mps_chi_left_[i + 1];
        int chi_R = mps_chi_right_[i + 1];
        ...
        // 3-step contraction (similar pattern but reversed)
        // Step 1: temp1[a, s, w', b'] = sum_b A[a, s, b] * R[b, w', b']
        {
            int64_t extentA[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)chi_R};
            int64_t extentR[] = {(int64_t)chi_R, (int64_t)D_mpo_, (int64_t)chi_R};
            int64_t extent_temp1[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)D_mpo_, (int64_t)chi_R};

            int32_t modesA[] = {0, 1, 2};
            int32_t modesR[] = {2, 3, 4};
            int32_t modes_temp1[] = {0, 1, 3, 4};

            hiptensor_contract(d_A, 3, extentA, modesA,
                             d_R_in, 3, extentR, modesR,
                             d_temp1, 4, extent_temp1, modes_temp1,
                             1.0, 0.0);
        }

        // Step 2: temp2[a, w, s', b'] = sum_{s,w'} temp1[a, s, w', b'] * W[w, s, s', w']
        {
            int64_t extent_temp1[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)D_mpo_, (int64_t)chi_R};
            int64_t extentW[] = {(int64_t)D_mpo_, (int64_t)d_, (int64_t)d_, (int64_t)D_mpo_};
            int64_t extent_temp2[] = {(int64_t)chi_L, (int64_t)D_mpo_, (int64_t)d_, (int64_t)chi_R};
        ...
        // Step 3: R_new[a, w, a'] = sum_{s',b'} temp2[a, w, s', b'] * A*[a', s', b']
        {
            int64_t extent_temp2[] = {(int64_t)chi_L, (int64_t)D_mpo_, (int64_t)d_, (int64_t)chi_R};
            int64_t extentA[] = {(int64_t)chi_L, (int64_t)d_, (int64_t)chi_R};
            int64_t extent_R_new[] = {(int64_t)chi_L, (int64_t)D_mpo_, (int64_t)chi_L};

            int32_t modes_temp2[] = {0, 1, 2, 3};
            int32_t modesA[] = {4, 2, 3};
            int32_t modes_R_new[] = {0, 1, 4};

            hiptensor_contract(d_temp2, 4, extent_temp2, modes_temp2,
                             d_A, 3, extentA, modesA,
                             d_R_out, 3, extent_R_new, modes_R_new,
                             1.0, 0.0);
        }
