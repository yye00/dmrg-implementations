...
def test_prepare_candidate_mps_list_reduced_sites():
    ...
    # Results are now (list[ndarray], energy) tuples.
    all_results = {}
    for site in range(L):
        upd = create_random_mps(L, 4, phys_dim=2, dtype="float64", canonical="left")
        all_results[site] = (extract_mps_arrays(upd), float(site))  # monotone energies
    ...
    # Each candidate should be a list of ndarray
    assert isinstance(cand[0], list)
    assert len(cand[0]) == L
