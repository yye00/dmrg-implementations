...
void build_josephson_mpo(int L, int d, int D_mpo,
    ...
    auto scale_op = [&](Complex c, const std::vector<Complex>& op) {
        ...
        for (int i = 0; i < d * d; i++) {
            result[i] = make_cuDoubleComplex(
                cuCreal(c) * cuCreal(op[i]) - cuCimag(c) * cuCimag(op[i]),
                cuCreal(c) * cuCimag(op[i]) + cuCimag(c) * cuCreal(op[i]));
        }
        return result;
    };
...
int test_heisenberg(int L, int chi_max, int n_outer, int n_segments,
    ...
    if (!quiet) {
        ...
        printf("  SVD: %s%s\n", gpu_svd ? "GPU (cusolver)" : "CPU (LAPACK)",
...
int main(int argc, char** argv) {
    ...
    try {
        if (run_josephson) {
            ...
            return test_heisenberg(L, chi_max, n_outer, n_segments, n_local, n_warmup, gpu_svd, use_rsvd, quiet);
        }
