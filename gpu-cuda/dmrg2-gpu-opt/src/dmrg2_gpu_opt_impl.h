...
DMRG2GPUOpt<Scalar>::DMRG2GPUOpt(int L, int d, int chi_max, int D_mpo, double tol)
    ...
    CUDA_CHECK(cudaMalloc(&d_ns_U_,     (size_t)ns_max * ns_max * sizeof(Scalar)));
    CUDA_CHECK(cudaMalloc(&d_ns_U_new_, (size_t)ns_max * ns_max * sizeof(Scalar)));
    CUDA_CHECK(cudaMalloc(&d_ns_gram_,  (size_t)ns_max * ns_max * sizeof(Scalar)));
    CUDA_CHECK(cudaMalloc(&d_ns_P_,     (size_t)ns_max * ns_max * sizeof(Scalar)));
        ...
        for (int s2p = 0; s2p < d; s2p++) {
            for (int n = 0; n < D; n++) {
                ...
                CUBLAS_CHECK(Traits::gemm_strided_batched(cublas_h_,
                    ...
                    R_env + n * cR, cR * D, (long long int)0,
                    &beta, d_result + s2p * cL * d, cL * dd, (long long int)cL, d));
            }
        }
    } else {
        for (int s1p = 0; s1p < d; s1p++) {
    ...
    if (m >= n_svd) {
        ...
        if (direction == 'R') {
            ...
            CUDA_CHECK(cudaMemcpyAsync(d_mps_tensors_[site], d_svd_U_,
                        m * new_k * sizeof(Scalar), cudaMemcpyDeviceToDevice, stream_));
            ...
            CUDA_CHECK(cudaMemcpyAsync(d_mps_tensors_[site + 1], h_Vh_trunc.data(),
                        new_k * n_svd * sizeof(Scalar), cudaMemcpyHostToDevice, stream_));
        }
...
void DMRG2GPUOpt<Scalar>::svd_split_fallback(int site, Scalar* d_theta, char direction) {
    ...
    if (direction == 'R') {
        ...
        allocate_mps_tensor(site + 1, new_k, cR);
        CUDA_CHECK(cudaMemcpy(d_mps_tensors_[site + 1], h_svd_tmp_.data(),
                            new_k * n_svd * sizeof(Scalar), cudaMemcpyHostToDevice));
        ...
        allocate_mps_tensor(site + 1, new_k, cR);
        if (new_k == full_k) {
            CUDA_CHECK(cudaMemcpy(d_mps_tensors_[site + 1], h_Vh_data,
