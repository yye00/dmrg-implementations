...
private:
    ...
    // === CUDA streams and cuBLAS/cuSOLVER handles ===
    ...
    int n_workers_;
    std::vector<std::vector<cudaStream_t>> worker_streams_;     // [segment][worker]
    std::vector<std::vector<cublasHandle_t>> worker_handles_;   // [segment][worker]
    std::vector<std::vector<cudaEvent_t>> worker_done_events_;  // [segment][worker]
    std::vector<cudaEvent_t> step_done_events_;                 // [segment]
    ...
    struct StreamWorkspace {
        ...
        Scalar* d_dav_AV;       // (theta_size_max, max_subspace) H x basis
        ...
        int* d_svd_info;
        Scalar* d_svd_work;
        int svd_lwork;          // cuSOLVER SVD workspace size
        ...
        Scalar* d_rsvd_qr_work; // QR workspace
        int rsvd_qr_lwork;      // QR workspace size
        int rsvd_orgqr_lwork;   // orgqr workspace size
