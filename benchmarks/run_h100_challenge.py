#!/usr/bin/env python3
"""
H100 Challenge Benchmarks — Realistic quantum computing scale.

44 configs across 3 models, targeting chi=64-512 and L=16-500.
GPU warmup before each timed run. All 6 GPU implementations + quimb CPU.

Usage:
    python3 benchmarks/run_h100_challenge.py                    # all impls, all configs
    python3 benchmarks/run_h100_challenge.py --impl dmrg-gpu    # single impl
    python3 benchmarks/run_h100_challenge.py --model heisenberg # single model
"""
...
def run_all(impl_names, models, do_warmup=True):
    ...
    for i, (impl, model, L, chi, sweeps) in enumerate(configs):
        ...
        r.update({
            ...
            "sweeps": sweeps, "arch": "h100", "benchmark": "challenge",
            "threads": 1, "np": 1,
        })
        if model == "josephson":
...
def main():
    parser = argparse.ArgumentParser(description="H100 Challenge Benchmarks")
    ...
    print(f"\n{'#'*65}")
    print(f"  H100 Challenge Benchmarks — Quantum Computing Scale")
