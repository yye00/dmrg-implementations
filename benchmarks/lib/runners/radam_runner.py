    ...
    **kwargs,
    ...
    Args:
        ...
        lr, epochs_per_sweep, min_lr: R-Adam hyperparameter overrides.
            Per-model defaults are in ``_DEFAULT_HYPERPARAMS``.
    ...
    defaults = _DEFAULT_HYPERPARAMS.get(model)
    if defaults is None:
        raise ValueError(
    ...
    epochs_per_sweep = (
        epochs_per_sweep if epochs_per_sweep is not None else defaults["epochs_per_sweep"]
    )
    min_lr = min_lr if min_lr is not None else defaults["min_lr"]
    ...
    max_epochs = max(int(epochs_per_sweep * max_sweeps), 50)
    ...
    return {
        ...
        "time": elapsed,
        "sweeps": result["epochs"],        # re-purposed -- see docstring
