...
private:
    ...
    // Randomized truncated SVD (Halko-Martinsson-Tropp) workspace
    bool use_rsvd_;
    int rsvd_oversampling_;
    Scalar* d_rsvd_omega_;    // (n, r) random projection matrix on GPU
    Scalar* d_rsvd_Y_;        // (m, r) projected result on GPU
    Scalar* d_rsvd_Q_;        // (m, r) QR factor on GPU
    Scalar* d_rsvd_B_;        // (r, n) projected matrix on GPU
    Scalar* d_rsvd_ipiv_;     // (r) QR pivot/tau on GPU (rocSOLVER)
    Scalar* d_rsvd_U_full_;   // (m, r) for U = Q @ U_small on GPU
    std::vector<Scalar> h_rsvd_B_;
    std::vector<Scalar> h_rsvd_U_small_;  // (r, r) from SVD of B
    ...
    void rsvd_and_update_mps(int site, Scalar* d_theta, char direction);
